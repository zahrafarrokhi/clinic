...
import { loadPayments } from "../../lib/slices/payment";
...
const Payments = () => {
  ...
  //pagination
  const LIMIT = 10;
  const count = useSelector((state) => state.paymentReducer?.payments?.count);
  const [offset, setOffset] = useState(0);
  const current_page = useMemo(() => offset / LIMIT + 1, [offset]);
  const number_of_pages = useMemo(() => Math.ceil(count / LIMIT), [count]);
  //redux
  const payments = useSelector((state) => state.paymentReducer?.payments?.results);
  ...
  //   []
  // );
  const getPayments = useCallback(async ({ search }) => {
    try {
      await dispatch(
        loadPayments({
          //pagination
  ...
  }, [offset, LIMIT, orderBy, order, start, end, status,search])

  useEffect(() => {
    getPayments({  });
  }, [offset,orderBy, order, start, end, status,search]);
  // ordering
        ...
        <Table>
          <TableHead className="hidden md:table-header-group">
            <TableRow>
              ...
              <StyledTableCell align="center">شرح</StyledTableCell>
              <CustomTableSortLabel
                order={order}
                orderBy={orderBy}
                id={"id"}
